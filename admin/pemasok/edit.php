...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $stmt = mysqli_prepare($koneksi, "UPDATE pemasok SET nama=?, no_hp=?, alamat=? WHERE id_pemasok=?");
        mysqli_stmt_bind_param($stmt, 'sssi', $old['nama'], $old['no_hp'], $old['alamat'], $id);
        if (mysqli_stmt_execute($stmt)) {
            $_SESSION['flash_message'] = 'Data pemasok berhasil diperbarui.';
...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-surface-container-lowest rounded-xl card-shadow border border-outline-variant/10 p-lg">
        <div class="flex items-center gap-sm mb-lg border-b border-outline-variant/10 pb-md">
            ...
            <div>
                <h3 class="text-[18px] font-semibold text-on-surface">Edit Pemasok</h3>
                <p class="text-[12px] text-on-surface-variant">#PM<?= str_pad($id, 3, '0', STR_PAD_LEFT) ?></p>
            </div>
        </div>
