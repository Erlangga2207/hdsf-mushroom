...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $stmt = mysqli_prepare($koneksi, "INSERT INTO pedagang (nama, no_hp, alamat) VALUES (?, ?, ?)");
        mysqli_stmt_bind_param($stmt, 'sss', $old['nama'], $old['no_hp'], $old['alamat']);
        if (mysqli_stmt_execute($stmt)) {
            $_SESSION['flash_message'] = 'Pedagang "' . $old['nama'] . '" berhasil ditambahkan.';
...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-surface-container-lowest rounded-xl card-shadow border border-outline-variant/10 p-lg">
        <div class="flex items-center gap-sm mb-lg border-b border-outline-variant/10 pb-md">
            ...
            <h3 class="text-[18px] font-semibold text-on-surface">Form Tambah Pedagang</h3>
        </div>
        ...
        <form method="POST" class="space-y-md">
            <div>
                ...
                <p class="text-[12px] text-on-surface-variant mt-xs">Pastikan nama sesuai dengan identitas resmi toko.</p>
            </div>
            ...
            <div>
                ...
                <p class="text-[12px] text-on-surface-variant mt-xs">Nomor HP aktif untuk notifikasi stok.</p>
            </div>
            ...
            <div>
                ...
                <textarea name="alamat" rows="4" placeholder="Jl. Jamur Barat No. 45, Kompleks Pertanian, Kota Bandung"
                ...
                <p class="text-[12px] text-on-surface-variant mt-xs">Akan digunakan untuk sistem logistik pengiriman.</p>
            </div>

            <!-- Panduan -->
            <div class="bg-surface-container-high/50 p-md rounded-lg border border-outline-variant/20">
                <div class="flex items-center gap-sm mb-sm">
                    <span class="material-symbols-outlined text-primary text-[18px]">info</span>
                    <h4 class="text-[13px] font-bold text-on-surface">Panduan Input</h4>
                </div>
                <ul class="space-y-xs text-[12px] text-on-surface-variant">
                    <li class="flex gap-xs"><span class="material-symbols-outlined text-[14px] mt-xs text-primary">check_circle</span><span>Nama pedagang harus sesuai identitas resmi toko.</span></li>
                    <li class="flex gap-xs"><span class="material-symbols-outlined text-[14px] mt-xs text-primary">check_circle</span><span>Nomor HP aktif untuk pengiriman notifikasi stok.</span></li>
                    <li class="flex gap-xs"><span class="material-symbols-outlined text-[14px] mt-xs text-primary">check_circle</span><span>Alamat lengkap untuk sistem logistik pengiriman jamur.</span></li>
                </ul>
            </div>
